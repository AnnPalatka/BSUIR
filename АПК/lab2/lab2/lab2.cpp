...
void print()
{
	...
	outp(0x20, 0x0A);
	val = inp(0x20);                    // get Vedusch zapros register                 
	for (i = 0; i < 8; i++)
	{
		temp = val % 2;
		val = val >> 1;
	...
	outp(0xA0, 0x0A);
	val = inp(0xA0);                     // get Vedom zapros register                
	for (i = 0; i < 8; i++)
	{
		temp = val % 2;
		val = val >> 1;
	...
	outp(0x20, 0x0B);
	val = inp(0x20);                     // get Vedusch obslug register                
	for (i = 0; i < 8; i++)
	{
		temp = val % 2;
		val = val >> 1;
	...
	outp(0xA0, 0x0B);
	val = inp(0xA0);                       // get Vedom obslug register              
	for (i = 0; i < 8; i++)
	{
		temp = val % 2;
		val = val >> 1;
	...
	val = inp(0x21);                // get mask Vedusch registor                     
	for (i = 0; i < 8; i++)
	{
		temp = val % 2;
		val = val >> 1;
	...
	val = inp(0xA1);              // get mask Vedom registor                       
	for (i = 0; i < 8; i++)
	{
		temp = val % 2;
		val = val >> 1;
...
void initialize()
{
	...
	setvect(0xB0, newintC0);
	setvect(0xB1, newintC1);
	setvect(0xB2, newintC2);
	setvect(0xB3, newintC3);
	setvect(0xB4, newintC4);
	setvect(0xB5, newintC5);
	setvect(0xB6, newintC6);
	setvect(0xB7, newintC7);

	_disable(); // CLI

	// interrupt initializtion for Vedusch
	outp(0x20, 0x11);	//ICW1 - initialize Vedusch
	outp(0x21, 0x08);   //ICW2 - base vector for Vedusch
	outp(0x21, 0x04);	//ICW3 - the port bit of Slave (in binary format)
	outp(0x21, 0x01);	//ICW4 - default

	// interrupt initialization for Vedom
	outp(0xA0, 0x11);  //ICW1 - initialize Vedom	
	outp(0xA1, 0xB0);  //ICW2 - base vector for Vedom
	outp(0xA1, 0x02);  //ICW3 - the port number of connected port on Vedusch 
