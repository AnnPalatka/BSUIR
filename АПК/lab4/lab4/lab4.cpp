...
unsigned int registerArray[] = { 0x00, 0x02, 0x04, 0x07, 0x08, 0x09 }; // регистры счетчиков: сек, мин, час, дн.мес., мес, лет
int alarmOn = 0;
char* months[] =
{
    "JANUARY",
...
void setTime()
{
    ...
    unsigned int check;
    do {
        outp(0x70, 0xA); // выбор регистра А   // биты 7-обновление, 4-6-делитель фазы, 3-0-выбор частоты периодического прерывания
    ...
    //биты 7-запрещено обнов часов, 6-вызов периодич прерыв (IRQ8), 5-вызов прерыв при срабат будильника
    //4-вызов прерыв по окончании обнов времени, 3-включ генерация прямоугол импульсов
    //2- 1/0 формат даты и времени в двоичный/BCD
    //1- 1/0 24часовой/12часовой режим
    //0- автоматич перех на летнее время в апреле и октябре
