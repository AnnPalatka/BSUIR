...
void interrupt new9(...) {
	...
	if (c == 0x01) flag = 1; // если это ESC устанавливаем флаг завершения программы
	if (c != 0xFA && !flag) { // иначе выводим скан-код в шестнадцатиричной форме
		itoa(c, buf, 16);
		cputs("0x");
		cputs(buf);
		cputs(" ");
...
// отправляем маску в порт клавиатуры
	...
	if (i == 3) {
		...
		flag = 1; // установка флага завершения программы
	}
}